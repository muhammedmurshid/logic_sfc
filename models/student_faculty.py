...
class StudentFacultyClub(models.Model):
    ...
    session_type = fields.Selection(selection=[('question', 'Questions'), ('lecture', 'Lecture')],
                                    string="Session Type", default='lecture')
    ...
    coordinator_head = fields.Many2one('res.users', string="Coordinator's Head",
    ...
    state = fields.Selection(string="State", selection=[('draft', 'Draft'), ('confirm', 'Confirmed'),
                                                        ('sent_to_approve', 'Sent for Head Approval'),
                                                        ('approved', 'Head Approved'),
                                                        ('payment_request', 'Payment Requested'), ('paid', 'Paid'),
    ...
    def confirm_sfc(self):
        if not self.sessions:
            ...
            print(self.student_id.name, 'student')
            student = self.env['logic.students'].search([('id', '=', self.student_id.id)])
            list = []
            for session in self.sessions:
                list.append((0, 0, {
                    'sfc_start_time': session.start_datetime,
                    'sfc_end_time': session.end_datetime,
                    'sfc_topic': session.lecture_topic,
                    'sfc_duration': session.hours,
                }))
            student.write({
                'sfc_ids': list
            })
